...
class Problem(abc.ABC):
    def __init__(self, model: torch.nn.Module, criterion, optimizer_manager, dataset, training_process_handler,
                 device="cuda:0", name="", validation_batch_size=128):
        self._model = model
    ...
    def train(self, n_epoch=20, batch_size=128):
        train_batches = self._dataset.get_train_batches(batch_size)
    ...
    def get_validation_metrics(self):
        return self.evaluate_batches(self._dataset.get_validation_batches(self._validation_batch_size))

    def get_test_metrics(self):
        return self.evaluate_batches(self._dataset.get_test_batches(self._validation_batch_size))
